...
In the Python example on the left, we do not know where the list might be mutated. In the Rust example on the right, numbers is set to a vector. The `&` sign indicates it is being passed as a reference. We know [0, 1, 2, 3] will be printed because numbers vector is immutable. 
